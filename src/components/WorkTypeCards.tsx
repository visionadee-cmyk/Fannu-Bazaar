...
export default function WorkTypeCards({
  ...
  const items = CATEGORY_CONFIG
    .filter((c) => !filteredCategories || filteredCategories.includes(c.category))
    .map((c) => ({ category: c.category, filename: c.filename, blurb: c.blurb }))

  return (
    <div className={className}>
      {items.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-gray-200 bg-white p-6 text-center">
          <div className="text-sm font-semibold text-gray-900">No matching categories</div>
          <div className="mt-1 text-xs text-gray-500">Try a different search word.</div>
        </div>
      ) : (
              ...
              >
                <div className="flex flex-row items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`font-semibold text-sm sm:text-base ${active ? 'text-emerald-900' : 'text-gray-900'}`}>{item.category}</div>
